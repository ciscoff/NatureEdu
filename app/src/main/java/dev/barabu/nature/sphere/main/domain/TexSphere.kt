...
class TexSphere(
    ...
    private val vertexArray: TexSphereVertexArray
    ...
    private val sStep = 186f / texWidth     // horizontal texture step
    private val tStep = 322f / texHeight    // vertical texture step
    ...
    init {
        ...
        vertexArray = TexSphereVertexArray(
            ...
            repeat(upDiamonds.size - 1) { i ->
                add(
                    icosahedron.vertices[upDiamonds[i]].copy(
                        tex = Tex((2 * i) * sStep, tStep),
                    )
                )
                add(
                    icosahedron.vertices[upDiamonds[i + 1]].copy(
                        tex = Tex((2 * i + 2) * sStep, tStep),
                    )
                )
                add(
                    icosahedron.vertices[0].copy(
                        tex = Tex((i * 2 + 1) * sStep, 0f),
                    )
                )
