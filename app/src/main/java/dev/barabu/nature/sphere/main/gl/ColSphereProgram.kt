...
import dev.barabu.nature.sphere.globe.domain.ColSphere
import dev.barabu.nature.sphere.globe.domain.GlobeSphere

class ColSphereProgram(
    context: Context,
    radius: Float,
    isFlat: Boolean,
    vertexShaderResourceId: Int = R.raw.col_sphere_vertex_shader,
    fragmentShaderResourceId: Int = R.raw.col_sphere_fragment_shader
) : ShaderProgram(
    ...
    override val model: Model = ColSphere(radius = radius, isFlat = isFlat)
    ...
    private var uViewMatrixDescriptor: Int =
        glGetUniformLocation(programDescriptor, U_VIEW_MATRIX)

    private var uProjMatrixDescriptor: Int =
        glGetUniformLocation(programDescriptor, U_PROJECTION_MATRIX)
